...
const GITHUB_URL =
  'https://github.com/yelingfeng/vue-pandora/tree/master/packages'
...
module.exports = function (source, u) {
  ...
  if (!request.endsWith('/index.md')) return source
  ...
  const typingFilepath = path.resolve(
    __dirname,
    `../types/packages/${pkg}/${moduleName}/index.d.ts`,
  )

  if (!fs.existsSync(typingFilepath)) return source

  const typing = fs.readFileSync(typingFilepath, 'utf-8')

  if (!typing) return source
