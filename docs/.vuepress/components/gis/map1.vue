...
export default {
  ...
  methods: {
    ...
    addMarkers() {
      this.clearMap()
      this.setMapZoom()
      this.$refs.drawMap.addMarkers(this.dataArr)
    },
    // 标记点击事件
    markerClick({ e, data }) {
      console.log(e, data)
      this.$refs.drawMap.openInfoPanel(`${data.name} | ${data.local}`, e)
    ...
    // 重置地图层级
    setMapZoom() {
      this.$refs.drawMap.markerZoomAdapter(this.dataArr)
    },
    //绘制多边形 网格
    drawPolygon() {
      this.clearMap()
      ...
      this.$refs.drawMap.markerZoomAdapter(_.flattenDeep(arr))
      arr.forEach(item => {
        this.$refs.drawMap.drawPolygon(item)
      })
    },
    // 划线
    drawPolyline() {
      this.clearMap()
      this.setMapZoom()
      this.$refs.drawMap.drawPolyline(this.dataArr)
    },
    // 画区域
    drawOverlay() {
      this.clearMap()
      this.setMapZoom()
      this.$refs.drawMap.drawOverlay(this.dataArr)
      // 一条记录可以画圆
    ...
    drawCircle() {
      this.clearMap()
      this.setMapZoom()
      this.$refs.drawMap.drawCircle({ lng: '104.080989', lat: '30.657689', radius: 500 })
    },
    // 热力地图
    drawHeatMap() {
      this.clearMap()
      ...
      this.$refs.drawMap.markerZoomAdapter(data)
      this.$refs.drawMap.drawHeatMap({ data: data, heatMax: 100 })
    },
    // 绘制轨迹
    drawTrajectory() {
      this.clearMap()
      this.setMapZoom()
      this.$refs.drawMap.drawTrajectory(this.dataArr)
    },
    ...
    drawLushu() {
      ...
      this.$refs.drawMap.drawPlayBackBtn(option)
      this.$refs.drawMap.drawLushu(this.dataArr)
    },
    // 海量点
    drawPointCollection() {
      this.clearMap()
      ...
      this.$refs.drawMap.drawPointCollection(this.dataArr, opt)
      this.$refs.drawMap.drawPointCollection(arr1, opt1)
    },
    // 实时
    lushuReal() {
      this.$refs.drawMap.lushuReal()
    },
    // 回放
    lushuStart() {
      this.$refs.drawMap.lushuStart()
    },
    // 暂停
    lushuPause() {
      this.$refs.drawMap.lushuPause()
    },
    // 停止
    lushuStop() {
      this.$refs.drawMap.lushuStop()
    },
    // 打开编辑模式
    openEditorMode() {
    ...
    drawBaiduMapLayer() {
      ...
      this.$refs.drawMap.drawBaiduMapLayer(this.mapvData, options)
    },
    // mapv
    drawBaiduMapLayer2() {
      this.clearMap()
