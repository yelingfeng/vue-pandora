<template>
  <el-form
    ...
    :form="form"
    size="small"
    id="v_query_condition"
    class="v-query-condition"
    :style="{
  ...
  >
    ...
    >
      ...
      <OptComponent
        ...
        v-bind="opt"
        :opt="opt"
        :ref="opt.comp === 'v-select-table' ? `tselecttableref-${i}` : ''"
        :form="form"
...
export default {
  ...
  methods: {
    ...
    resetHandle() {
      ...
      // 获取所有的tselecttable
      const refList = Object.keys(this.$refs).filter(item => item.includes('tselecttableref'))
      if (refList.length > 0) {
        refList.map(val => {
